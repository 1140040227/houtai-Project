<template>
	<div>
		...
		<!-- 编辑参数对话框 -->
		<el-dialog :title=" '修改' + getTitleText" :visible.sync="editDialogVisible" width="50%"
		...
		methods: {
			...
			getCateList() {
				service({
					...
					if (res.data.meta.status == 400) {
						this.$lxmessage.error('获取商品数据列表失败!');
						return
					} else {
			...
			addParams() {
				this.$refs.addFromRef.validate(valid => {
					...
					service({
						...
						// console.log(res);
						if (res.data.meta.status == 400) {
							return this.$lxmessage.error('添加参数失败');
						}
						this.$lxmessage.success('添加参数成功!');
			...
			showEditDialog(attrId) {
				service({
					...
					// console.log(res);
					if (res.data.meta.status == 400) {
						return this.$lxmessage.error('获取分类失败！');
					}
			...
			editParams() {
				this.$refs.editFromRef.validate(valid => {
					...
					service({
						...
						// console.log(res);
						if (res.data.meta.status == 400) {
							return this.$lxmessage.error('修改参数失败！');
						}
						this.$lxmessage.success('修改参数成功！')
						this.getParamsData()
			...
			async removeParams(attrId) {
				this.$confirm('此操作将永久删除该参数，是否继续？', '提示', {
					...
					service({
						...
						// console.log(res);
						if (res.data.meta.status == 400) {
							return this.$lxmessage.error('删除参数失败');
						}
						this.$lxmessage.success('删除参数成功！')
						this.getParamsData()
					})
				}).catch(() => {
			...
			saveAttrVals(row) {
				service({
					...
					// console.log(res);
					if (res.data.meta.status == 400) {
						return this.$lxmessage.error('修改参数项失败！')
					}
					this.$lxmessage.success('修改参数项成功！')
				})
			},
			// 点击按钮显示输入框
			showInput(row) {
