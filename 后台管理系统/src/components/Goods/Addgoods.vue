		...
		methods: {
			...
			tabClicked() {
				...
				if (this.activeIndex === '1') {
					service({
						...
						// console.log(res);
						if (res.data.meta.status == 400) {
							return this.$lxmessage.error("获取动态参数列表失败");
						}
			...
			addGoods() {
				this.$refs.addFormRef.validate(async valid => {
					if (!valid) return this.$lxmessage.error('请填写必要的表单项！')
					...
					service({
						...
						console.log(res);
						if (res.data.meta.status == 400) {
							return this.$lxmessage.error('添加商品失败');
						}
						this.$lxmessage.success('添加商品成功!')
